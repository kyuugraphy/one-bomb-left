...
- **`G` force-spawns an `iron_plating` pickup — DEBUG, temporary.** Walk onto it once to take it, again to see the already-owned path. It logs a `console.warn` on startup so it cannot be forgotten quietly; see the cleanup TODO.
...
## TODO — cleanup (debug code currently in the build)
- [ ] **Remove the `G` debug item spawner from `src/game/PlayScene.js`.** It force-spawns a clean `iron_plating` reward pickup 84 px from the player so the duplicate-item path can be walked through on demand. Delete: the `DEBUG_SPAWN_KEY` / `DEBUG_SPAWN_ITEM_ID` / `DEBUG_SPAWN_OFFSET` constants and the `console.warn` beside them, the `debugSpawnKey` binding in `create()`, the `JustDown` check in `update()`, and the `debugSpawnItemPickup` / `debugFreeSpotNearPlayer` methods. Every piece sits between `===== DEBUG` and `===== end DEBUG` markers, and flipping `DEBUG_SPAWN_KEY` to `false` disables the lot in one edit if you only want it quiet for a build.
