...
**A DPI pass was built for it and then reverted.** The mismatch was real - `RENDER_SCALE`, a larger canvas and a matching camera zoom did make the frame land 1:1 on physical pixels - and it changed nothing anyone could see, so it was taken back out rather than left in for the fill rate. Canvas is 1344x840 again and the camera is unzoomed. If it is ever wanted, the shape of it is in `7051ede`: the world keeps its coordinate space, `this.scale.width/height` become canvas pixels rather than world units, and every Text object needs building at the higher resolution or the HUD comes out softer than it started.

**Rocks are back to `rock_tight.png` at the plain cell size.** `rock_org.png` was tried and is still in `public/sprites` but nothing loads it: the untrimmed export's art stops short of its frame - measured at 0.926 of the width - and `TILE_TURN_COVER` assumes a tile that reaches the edge, so the rolled turn bared the cell corners and a clump read as separate stones with floor between them. Overdrawing 1.15x closed that, and both halves were reverted together. **The measurement is the part worth keeping:** any replacement rock art wants trimming to its frame, or the cover factor stops meaning what it says.
