...
**Still open, and the leading suspect is `Scale.FIT` itself.** The canvas stays 1344x840 and the browser CSS-scales the bitmap to the window, so a scrolling scene is resampled differently every frame - static content reads as acceptably soft, motion does not. `Scale.NONE` sits in the working tree as a diagnostic to confirm or kill that, deliberately uncommitted. If it is confirmed, the fix is to render at the display's resolution and scale the world with camera zoom rather than scaling the canvas with CSS - which is the same **zoom the camera or scale the world** fork the third pass wrote down, and it changes how much of a room is visible. That makes it a decision to be taken, not a bug to be closed.
