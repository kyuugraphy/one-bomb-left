...
**Playtest verdict: the rule is right, and it exposed a rendering fault older than itself.** Threading gaps works and open floor is left alone. But **wherever the camera scrolls, the scene blurs while it moves** - and gap assist is what made that constant rather than occasional, because it is the first thing that moves her perpendicular during ordinary straight-line walking. Before it, the camera only lerped on two axes on a diagonal or a sprint.

**The blur maps exactly onto "can the camera scroll".** A standard room is 1344x840, which *is* the viewport, so `cameraBoundsFor` leaves the camera no range and nothing moves - and nothing blurs. Big rooms blur. Long corridors blur. Two explanations were ruled out by reading rather than by trying: **sub-pixel sprite positions**, because `startFollow(player, true, ...)` was already setting the camera's own `roundPixels` and at zoom 1 that rounds every textured draw; and **the lean oscillating**, because it closes about 28.6% of the remaining offset per frame, which decays onto the centreline instead of hunting around it.

**Camera pixel-snapping is off now, and it was not the cause.** `startFollow`'s second argument is `roundPixels`, and it is `false`. Snapping only pays off when the canvas maps 1:1 to device pixels at an integer zoom, and `Scale.FIT` rescales 1344x840 by whatever non-integer factor the window needs, so the crispness it protects is resampled away one step later regardless. Nothing here is pixel art to protect either - every tile is drawn rotated a rolled few degrees and every avatar pose is scaled to a held height, so all of it is linearly filtered already. The reasoning stands on its own and the reason is written at the call site, but **it did not fix the blur**, so it is recorded here as a cleanup and not as a repair.

**Still open after five explanations, and this is the list of what it is not.** Each was ruled out by measuring, not by trying a fix and squinting:

- **Camera pixel-snapping.** `startFollow`'s second argument was already `true`; turning it off changed nothing. Left off anyway - see above.
- **The `Scale.FIT` rescale.** Tried `Scale.NONE`, rendering the canvas at native size with no CSS scaling at all. No change. Reverted.
- **Missing mipmaps.** Added `render: { mipmapFilter: 'LINEAR_MIPMAP_LINEAR' }`. A probe read back `minFilter: LINEAR_MIPMAP_LINEAR, isMipmap: true` on `wall_tight_round`, the texture the room is really drawn with - so it worked exactly as intended and made no visible difference.
- **Texture minification.** Every asset is stored at 1024-1254 px and drawn at 46-75, so each screen pixel covers ~15x15 texels. Real, and not what is being seen.
- **The DPI mismatch.** The probe found `devicePixelRatio: 2` with a 1344x840 canvas presented across 2720x1700 physical pixels - the frame genuinely was being upscaled 2x. Fixed properly (below). Still no change.

**The DPI pass landed regardless, because the mismatch was real even if it is not the bug.** `RENDER_SCALE` is `min(ceil(devicePixelRatio), 2)`, the canvas is built that many times larger, and the camera is zoomed to match - so **the world keeps its 1344x840 coordinate space** and not one room, hitbox or tuned number moves. Two things follow and are easy to forget: `this.scale.width/height` are canvas pixels rather than world units, so everything laying out against the screen edge goes through `viewSize()`; and a zoomed camera scales a Text object's texture like any other sprite, so all 18 text objects are built through `crispText()` at the same resolution or the HUD would come out softer than before the zoom existed. It is a no-op on an ordinary 1x display.

**What is left to try, for whoever picks this up:** the paused frame is crisp and the moving one is not, on a canvas that now maps 1:1 to physical pixels, with correct mipmaps. That combination does not point at a texture or a scale setting any more. It points at the motion itself - sample-and-hold on the display, or the 0.12 camera lerp resampling the scene every frame at a fractional scroll. A camera deadzone would test the second cheaply, and would also stop gap assist moving the camera for a 3 px drift, which is what made the blur constant rather than occasional in the first place.

**Rocks are drawn from `rock_org.png` now, overdrawn 1.15x.** The untrimmed export's art stops short of its frame - measured at 0.926 of the width - and `TILE_TURN_COVER` assumes a tile that reaches the edge, so the rolled turn bared the cell corners and a clump read as separate stones with floor showing between them. `1/0.926 = 1.08` restores what `rock_tight.png` drew and the remainder is deliberate overlap. Visual only: `paintShape` puts the body back to its cell either way. Note it is 1254 px and so not a power of two, which means it alone gets no mip chain.
...
## Not done / known gaps
- **Most of the avatar's detail does not survive the downscale, and that is a drawing decision rather than a bug.** The art is 1254x1254, the figure trims to 707x1238, and it is drawn 46 px tall - a **27x reduction**, landing at roughly **26x46 px** on screen. Checked by downscaling each pose exactly as the game does and magnifying the result with smoothing off:
  - **Silhouette and palette survive well.** The red/black/gold reads instantly, the hair mass is distinctive, and left and right are easy to tell apart by the hair sweep and the drape.
  - **The jewelry is gone.** Gold pieces become isolated bright pixels that read as sparkle or noise rather than as jewelry.
  - **The face is two or three pixels.** Eyes are single pixels: it reads as a face on `down` and as mush on `left` and `right`.
  - **`up` is the weakest pose** - a dark hair mass, hard to tell from `down` at a glance while moving, because hair dominates both.

  Two ways out if the detail should count: draw the player larger - 60-70 px would recover the face - or accept it and treat these as source art for portraits and UI, where they are seen at a size that earns the linework.
- **`public/sprites/` is 17.5 MB across 14 files**, of which 3.6 MB is the four player poses. That is a real first-load cost for a web game, and it grows fastest with player frames - see the walk-cycle entry in `zz_todo.md`.
